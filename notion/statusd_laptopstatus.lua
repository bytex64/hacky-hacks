...
local function get_battery()
  ...
  if pcall(function ()
       ...
       status, _, voltage = string.find(statecontents, "present voltage:%s+(%d+).*")
       if not status then error("could not get battery voltage") end
       voltage = tonumber(voltage) / 1000.0
     end) then
       ...
       return { percent=string.format("%02d%%", percent), timeleft=timeleft, percenthint=percenthint, timelefthint=timelefthint, voltage=string.format("%0.1fV",voltage)}
  else return { percent="n/a", timeleft="n/a", percenthint=percenthint, timelefthint=timelefthint, voltage=string.format("%0.1fV",voltage)} end
end
...
local function update_laptopstatus ()
  ...
  if statusd ~= nil then
    ...
    statusd.inform("laptopstatus_batteryvoltage", battery.voltage)

...
defbindings("WScreen", {
    ...
    bdoc("Run a command based on the X selection"),
    kpress(META.."V", "ioncore.exec('selection-oracle')"),
...
defbindings("WMPlex.toplevel", {
    ...
    bdoc("Query for host and program via SSH w/remote X."),
    kpress(ALTMETA.."Shift+F4", 
           "mod_query.query_ssh(_, ':ssh -Y')"),

    bdoc("Query for work host"),
    kpress(ALTMETA.."Control+F4",
           "mod_query.query_ssh(_, ':ssh-work')"),

    bdoc("Read new mail in mutt."),
    kpress(ALTMETA.."F5", "mod_query.exec_on_merr(_, ':mutt -Z')"),

    bdoc("List folders in mutt."),
    kpress(ALTMETA.."Shift+F5", "mod_query.exec_on_merr(_, ':mutt -y')"),

    bdoc("Fetch mail."),
    kpress(ALTMETA.."Control+F5", "mod_query.exec_on_merr(_, ':offlineimap')"),

    bdoc("Open ROX-Filer"),
    kpress(ALTMETA.."F6", 
           "mod_query.query_runfile(_, 'rox-filer', 'Directory:')"),

    bdoc("Open URL"),
    kpress(ALTMETA.."F7",
           "mod_query.query_uri(_, 'web-browser')"),

    bdoc("Query via namazu"),
    kpress(ALTMETA.."F8", "mod_query.query_namazu(_, 'bnamazu-permanence-hack')"),

    bdoc("Query for workspace to go to or create a new one."),
    kpress(ALTMETA.."F9", "mod_query.query_workspace(_)"),

    bdoc("Open a set of windows for a cluster"),
    kpress(ALTMETA.."F10", "mod_query.query_cluster(_)"),

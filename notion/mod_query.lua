...
function mod_query.query_ssh(mplex, ssh)
    ...
    local function handle_exec(mplex, str)
        ...
        -- Chip - Fix up SSH arguments
        local st, en, addr, cmd = string.find(str, "^([^ ]+) ?(.*)$")
            
        if string.len(cmd) > 0 then
            mod_query.exec_on_merr(mplex, ssh.." "..string.shell_safe(addr).." "..string.shell_safe(cmd))
        else
...
function mod_query.query_uri(mplex, browser)
    browser = (browser or "firefox")

    local function handle_exec(mplex, str)
        mod_query.exec_on_merr(mplex, browser.." "..str)
    end

    return mod_query.query(mplex, TR("Open URI:"), nil, handle_exec, nil, "uri")
end
...
function mod_query.query_namazu(mplex, browser)
    local function handle_namazu(mplex, str)
        mod_query.exec_on_merr(mplex, "bnamazu -b "..browser.." '"..str.."'")
    end

    return mod_query.query(mplex, TR("Query:"), nil, handle_namazu, nil, "namazu")
end

function mod_query.query_cluster(mplex)
    local function handle_cluster(mplex, name)
        local ws = ioncore.lookup_region(name, "WGroupWS")
        if ws then
            ws:goto()
        else
            ...
            if not ioncore.create_ws(scr, tmpl, "cluster") then
                error(TR("Unknown error"))
            end

            mod_query.exec_on_merr(mplex, "work-cluster " .. name)
        end
    end

    return mod_query.query(mplex, TR("Cluster:"), nil, handle_cluster, nil, "cluster")
end
-- }}}
